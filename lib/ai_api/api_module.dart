...
class AsyncService {
  ...
  void addDataCache(List<Word> wordList) async {
    final List<Map<String, String>> _cacheList = List.generate(_dataList.length, (i) {
      return _dataList[i].toSimpleJson();
    });
    await addCacheData(_cacheList).then((value) async {
      print('----add cache success----');
      final prefs = await SharedPreferences.getInstance();
      List<String> cacheKeys = prefs.getStringList('cacheKeys')?? [];
      print('now cacheCount: ${cacheKeys.length}');
    });
  }
    ...
    _receivePort.listen((response) {
      try {
        ...
        if(dataList.isNotEmpty) {
          _dataList = dataList;
          ///json_cache 저장 모듈 호출
          addDataCache(_dataList);
        }
        print('[callApiProcess] dataList: $_dataList}'); //결과 출력
      } catch(e) {
        throw e;
      } finally {
  ...
  static _loadData (dynamic args) async {
  ...
  static Future<List<Word>> getWordListFromImages(List<Uint8List> params, List<String> types) async {
    ...
    if(params.length == types.length) {
      for(int i=0; i < params.length; i++) {
        var data = params[i];
        var type = types[i];
        List<Word> words = await getApiReponseAsWord(data, type);
  ...
  static Future<List<Word>> getApiReponseAsWord(Uint8List data, String type) async {
    ...
    detectedWords = detectedWords.toSet().toList(); ///중복 데이터 제거
