...
class Alert{
  ...
  manageAlertSettings(BuildContext context) {
    Future.delayed(Duration.zero, () async {
      ...
      List<bool> checkList = List<bool>.generate(notifList.length, (index) => true);
                    ...
                    Container(
                        ...
                        child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int idx) {
                              return NotifCard(notifInfo: notifList[idx], active: checkList[idx]);
                            }
                        )
                    ),
                  ],
                      ...
                      children: <Widget>[
                        ButtonTheme(
                          child: DialogButton(
                            ...
                            child: Text('SAVE', style: TextStyle(color: Colors.white, fontSize: 13.sp)),
                            onPressed: () {
                              turnOffList.forEach((e) => turnOff(e));
                              turnOnList.forEach((e) { });
                              alert.onSuccess(context, "알림이 저장되었습니다.");
                            },
                          ),
                        ),
                        ButtonTheme(
...
class NotifCard extends StatefulWidget {
  ...
  final bool active;
  NotifCard({Key? ke, required this.notifInfo, required this.active});
