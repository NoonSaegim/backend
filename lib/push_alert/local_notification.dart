...
Future<void> initWorkManager(int seq, String uid, VoidCallback taskExecutor, String taskName, String? summary, bool repeat, Duration duration) async {
  ...
  if(repeat) {
    ...
    .then((value) async {
      ...
      notifList.add('$taskName:$seq:$summary:$uid'); /// 나중에 알림 설정 해지 및 복구를 위해서 'taskName, 글번호, 알림 시간 요약, uid' 저장!
...
Future<void> setNewAlert(int seq, String title, String noteKey, String time, String cycle, bool repeat) async {
  ...
  if(days.length == 1) {
    ...
    await initWorkManager(seq ,uid, callbackDispatcher, title, '$time $cycle', repeat, duration);
    ...
    if(duration?.length > 0){
      for(int i =0; i < duration.length; i++) {
        await initWorkManager(seq, Uuid().v4(), callbackDispatcher, '$title@$i', '$time $cycle',repeat, duration[i]);
      }
    }
  }
}
